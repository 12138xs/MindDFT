...
def main():
    ...
    device = "GPU" if options["enable_cuda"] else "CPU"
    ms.set_context(mode=ms.context.PYNATIVE_MODE, device_target=device)
    ...
    for epoch in range(options["max_epoch"]):
        ...
        if epoch % (n_restart) == n_restart-1:
            save_model(model, options["model_save_path"] + f".restart{epoch+1}")
