...
CONFIG_PATH1 = os.path.join(ROOT_DIR, '../Trained_models/weights_DOS.ckpt')

class single_atom_model(nn.Cell):
    def __init__(self, in_channels):
        super(single_atom_model, self).__init__()
        self.denseSeq = nn.SequentialCell(
            nn.Dense(in_channels, 600, activation='relu'),# weight_decay=0.1, kernel_initializer='glorot_uniform'
            nn.Dropout(p=0.9),
...
class init_DOSmod():
    ...
    def train(self, train_dataset, valid_dataset, epochs=100, filepath="newDOSmodel.ckpt"):
        ...
        def forward_fn(data, label):
            ...
            label1, label2 = label
            loss = self.loss_weights[0] * self.loss_fn(out1, label1) + self.loss_weights[1] * self.loss_fn(out2, label2)
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, validset in enumerate(valid_dataset.create_dict_iterator()):
                ...
                out1, out2 = self._network(X_val_C, X_val_H, X_val_N, X_val_O, X_el_val, C_dV, H_dV, N_dV, O_dV)
                err = self.loss_weights[0] * self.loss_fn(out1, Prop_dos_val) + self.loss_weights[1] * self.loss_fn(out2, vbcb_val)
