...
from . import layer
ShiftedSoftplus  = layer.ShiftedSoftplus
...
class DensityModel(nn.Cell):
    ...
    def consturct(self, 
                  ...
                  atom_xyz, 
                  cell):
        atom_representation = self.atom_model(nodes, atom_edges, atom_edges_displacement, num_nodes, num_atom_edges, atom_xyz, cell)
        probe_result = self.probe_model(nodes, probe_edges, probe_edges_displacement, num_nodes, num_probe_edges, num_probes, probe_xyz, atom_xyz, cell, atom_representation)
        return probe_result
...
class PainnDensityModel(nn.Cell):
    ...
    def construct(self, 
                  ...
                  atom_xyz, 
                  cell):
        atom_representation_scalar, atom_representation_vector = self.atom_model(nodes, atom_edges, atom_edges_displacement, num_nodes, num_atom_edges, atom_xyz, cell)
        probe_result = self.probe_model(nodes, probe_edges, probe_edges_displacement, num_nodes, num_probe_edges, num_probes, probe_xyz, atom_xyz, cell, atom_representation_scalar, atom_representation_vector)
        return probe_result
...
class ProbeMessageModel(nn.Cell):
    ...
    def construct_and_gradients(
        self,
        nodes,probe_edges, probe_edges_displacement, num_nodes, num_probe_edges, num_probes, probe_xyz, atom_xyz, cell,
        ...
        if compute_hessian:
            hessian_shape = (probe_xyz.shape[0], probe_xyz.shape[1], 3, 3)
            hessian = ops.zeros(hessian_shape, dtype=probe_xyz.dtype)
    ...
    def construct(
        ...
        atom_xyz = layer.batch_dim_reduction(atom_xyz)
        ...
        probe_edges_displacement = layer.batch_dim_reduction(probe_edges_displacement)
        ...
        probe_edges = probe_edges + edge_probe_offset
        ...
        probe_state_shape = (
            ops.sum(num_probes).asnumpy().item(),
...
class PainnAtomRepresentationModel(nn.Cell):
    ...
    def construct(self, 
                  ...
                  cell):
        # Unpad and concatenate edges and features into batch (0th) dimension
        edges_displacement = layer.batch_dim_reduction(atom_edges_displacement)
        edge_offset = ops.cumsum(
            ops.cat(
                (
        ...
        # Unpad and concatenate all nodes into batch (0th) dimension
        atom_xyz = layer.batch_dim_reduction(atom_xyz)
        nodes_scalar = layer.batch_dim_reduction(nodes)
...
class PainnProbeMessageModel(nn.Cell):
    ...
    def construct_and_gradients(
        ...
        if compute_hessian:
            hessian_shape = (probe_xyz.shape[0], probe_xyz.shape[1], 3, 3)
            hessian = ops.zeros(hessian_shape, dtype=probe_xyz.dtype)  # device=probe_xyz.device
            grad_function_dp2_dxyz2 = ms.grad(
