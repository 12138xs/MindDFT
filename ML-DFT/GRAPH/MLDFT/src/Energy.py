...
class forward_fn(nn.Cell):
    ...
    def construct(self, data, label, _network):
        out1, out2, out3, out4, out5, out6 = _network(*data)
    ...
    def train(self, train_dataset, valid_dataset, epochs=100, filepath="newEmodel.ckpt"):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, validset in enumerate(valid_dataset.create_dict_iterator()):
                ...
                err, logits = self.forward_fn([X_val_C,X_val_H,X_val_N,X_val_O,X_at_val,C_mV,H_mV,N_mV,O_mV], [ener_val,forces1V,forces2V,forces3V,forces4V,press_val], self._network)
            self._network.set_train(True)
