...
class forward_fn(nn.Cell):
    ...
    # ms.jit
    def construct(self, data, label, _network):
        out1, out2 = _network(*data)
        label1, label2 = label
...
class init_DOSmod():
    ...
    def train_step(self, data, label):
        (loss, logits), grads = self.value_and_grad(data, label, self._network)
        self.optim(grads)
        return loss, logits
    ...
    def train(self, train_dataset, valid_dataset, epochs=100, filepath="newDOSmodel.ckpt"):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, trainset in enumerate(train_dataset.create_dict_iterator()):
                ...
                input_ = ms.mutable([X_C,X_H,X_N,X_O,X_el,C_d,H_d,N_d,O_d])
                label_ = ms.mutable([Prop_dos,vbcb])
                loss, logits = self.train_step(input_,label_)
            ...
            for batch_idx, validset in enumerate(valid_dataset.create_dict_iterator()):
                ...
                input_ = ms.mutable([X_val_C,X_val_H,X_val_N,X_val_O,X_el_val,C_dV,H_dV,N_dV,O_dV])
                label_ = ms.mutable([Prop_dos_val,vbcb_val])
                err, _ = forward_fn()(input_, label_, self._network)
